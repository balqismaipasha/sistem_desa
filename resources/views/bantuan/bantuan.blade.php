...
@section('content')
    ...
    <div class="modal fade" id="modal-default">
        <div class="modal-dialog">
            <form class="form-horizontal" action="{{ url('/simpan_bantuan') }}" method="POST"
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-9">
                                    <input type="text" class="form-control" name="nama_bantuan"
                                        placeholder="Nama Bantuan" required>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <button type="button" class="btn btn-primary" id="tambah_data" data-toggle="modal"
                                data-target="#modal-default" title="Tambah Data Keluarga">
                                        ...
                                        <tr class="text-center">
                                            ...
                                            <td class="text-center py-0 align-middle">
                                                <div class="btn-group btn-group-sm">
                                                    <data class="d-none" id="id_{{ $values->id_bantuan }}">
                                                        {{ json_encode($values) }}</data>
                                                    <a href="" id="editBantuan" data-toggle="modal"
                                                        data-target="#edit-modal-{{ $values->id_bantuan }}"
                                                        class="btn btn-success">
                                                        <i class="fas fa-edit"></i>
                                                    </a>
                                                    <a href="{{ url('/hapus_bantuan/' . $values->id_bantuan) }}"
                                                        class="btn btn-danger">
                                                        <i class="fas fa-trash"
                                                            onclick="return confirm('Hapus Data Penduduk ?')">
                                                        </i>
                                                    </a>
                                                </div>
                                            </td>
    ...
    @foreach ($data_bantuan as $values)
        <div id="edit-modal-{{ $values->id_bantuan }}" class="modal fade" tabindex="-1" role="dialog"
            aria-labelledby="edit-modal-{{ $values->id_bantuan }}" aria-hidden="true">
            <div class="modal-dialog">
                <form id="companydata">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h4 class="modal-title">Ubah Data Bantuan</h4>
                        </div>
                        <div class="modal-body">
                            <div class="card-body">
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-9">
                                        <input type="hidden" name="id_bantuan" value="{{ $values->id_bantuan }}" />
                                        <input type="text" class="form-control" name="nama_bantuan"
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.submitedit').click(function() {
                var nama_bantuan = $(this).closest('.modal-content').find('[name="nama_bantuan"]').val();
                var id_bantuan = $(this).closest('.modal-content').find('[name="id_bantuan"]').val();

                $.ajax({
                    url: '{{ asset('ubah_bantuan') }}/' + id_bantuan,
                    type: "POST",
                    data: {
